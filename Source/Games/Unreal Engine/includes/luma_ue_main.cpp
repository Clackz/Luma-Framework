...
namespace
{
   ...
   // DLSS true super-resolution: cached downscaled source color RT. The game's
   // full-resolution source is StretchRect'd into this smaller RT, then the
   // small RT is fed to DLSS as pInColor so DLSS performs an actual 1.5x
   // upsample instead of DLAA. Recreated only when the target size changes.
   static com_ptr<ID3D11Texture2D> g_sr_source_color_scaled;
   static uint32_t                 g_sr_source_color_scaled_w = 0;
   static uint32_t                 g_sr_source_color_scaled_h = 0;
   ...
   DrawOrDispatchOverrideType OnDrawOrDispatch(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, reshade::api::shader_stage stages, const ShaderHashesList<OneShaderPerPipeline>& original_shader_hashes, bool is_custom_pass, bool& updated_cbuffers, std::function<void()>* original_draw_dispatch_func) override
   {
      ...
      if (is_taa && device_data.sr_type != SR::Type::None && !device_data.sr_suppressed)
      {
         ...
         if (dlss_inputs_valid)
         {
            ...
            // True super-resolution: source color is downscaled by 1/1.5
            // (DLSS Quality). The actual downscale pass is below, right after
            // the TAA shader source colour is fetched. Keeping the scaling
            // factor in device_data.sr_render_resolution_scale matches what
            // FF7 Remake's mod does, so the rest of the SR pipeline stays
            // generic and DLSS picks Quality automatically.
            device_data.sr_render_resolution_scale = 1.0f / 1.5f; // DLSS Quality (1.5x upsample)

            SR::SettingsData settings_data;
            settings_data.output_width = taa_output_texture_desc.Width;
            settings_data.output_height = taa_output_texture_desc.Height;
            // Match the downscaled source RT (see source blit pass below).
            // DLSS requires render dimensions to be a multiple of 64.
            settings_data.render_width  = ((uint32_t)(taa_output_texture_desc.Width  / 1.5f)) & ~63u;
            settings_data.render_height = ((uint32_t)(taa_output_texture_desc.Height / 1.5f)) & ~63u;
            ...
            if (!skip_dlss)
            {
               ...
               // DLSS accepts pInColor and render_width that differ: it samples
               // the source texture and treats render_width/height as the
               // logical input size. We exploit this by giving DLSS a physically
               // smaller source RT and declaring the matching render dimensions,
               // so DLSS upsamples the small RT to the TAA output size. That is
               // what "real" super-resolution looks like.
               //
               // Without this block the source color fed to DLSS is the full TAA
               // RT (1920x1080 in dungeon, 2560x1440 in town), so DLSS ends up
               // doing 1:1 temporal AA (DLAA) and the rendering cost is unchanged.
               {
                  com_ptr<ID3D11Texture2D> src_tex;
                  HRESULT hr_q = game_device_data.sr_source_color->QueryInterface(&src_tex);
                  if (SUCCEEDED(hr_q) && src_tex)
                  {
                     D3D11_TEXTURE2D_DESC src_desc;
                     src_tex->GetDesc(&src_desc);
                     const float scale = device_data.sr_render_resolution_scale; // 1/1.5
                     uint32_t target_w = ((uint32_t)(src_desc.Width  * scale)) & ~63u;
                     uint32_t target_h = ((uint32_t)(src_desc.Height * scale)) & ~63u;
                     if (target_w >= 64 && target_h >= 64)
                     {
                        if (!g_sr_source_color_scaled ||
                            g_sr_source_color_scaled_w != target_w ||
                            g_sr_source_color_scaled_h != target_h)
                        {
                           ...
                           g_sr_source_color_scaled = nullptr;
                           HRESULT hr_create = native_device->CreateTexture2D(&small_desc, nullptr, &g_sr_source_color_scaled);
                        ...
                        if (g_sr_source_color_scaled)
                        {
                           // StretchRect is the cheapest path that does both copy
                           // and bilinear-style scale in one hardware-accelerated call.
                           native_device_context->StretchRect(
                              game_device_data.sr_source_color.get(), 0,
                              g_sr_source_color_scaled.get(), 0,
                              nullptr);
                           game_device_data.sr_source_color = g_sr_source_color_scaled;
                        }
                     }
                  }
               }
